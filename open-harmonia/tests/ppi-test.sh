...
do 
    ...
    do 
        ...
        do 
            j=0
            ../bpt_test ../data_set/insert_$ts.txt ../data_set/search_${distr}.txt 10 > tmp
            ...
            do  
                ...
                if [ $j -eq 3 ]
                then 
                    echo "cpu_compute_time + $a"
                    cpu_compute_time=$(echo "scale=6; $cpu_compute_time + $a" | bc)
                fi 
                if [ $j -eq 4 ]
                then 
                    echo "cpu_total_time + $a"
                    cpu_total_time=$(echo "scale=6; $cpu_total_time + $a" | bc)
                fi 
        ...
        GPU_time=$(echo "scale=6;$GPU_time / 8" | bc)
        total_time=$(echo "scale=6;$total_time / $N" | bc)
        cpu_compute_time=$(echo "scale=6; $cpu_compute_time / $N" | bc)
        cpu_total_time=$(echo "scale=6;$cpu_total_time /$N" | bc)
