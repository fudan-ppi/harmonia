...
__global__ void multi_prefix_sort_1thread_noleaf_kernel(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys, BLeaf_node *h_leaf){
    /*{{{*/ 
        ...
        for(int j=0;j<GPU_SWP_SIZE1;j++){
            ...
            if(i==tree_height-1){
                if(isBalance){
                        ...
                        result.blfnode = (int)(((BLeaf_node *)(node->child[0]))-h_leaf);//useless
...
/*--------------------------------------------------------------------------
*
*       multi-prefix-sort-1thread-noleaf 
*       cpu dynamic
*
*
*-----------------------------------------------------------------------------*/
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            multi_prefix_sort_1thread_noleaf_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys_after[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket, bucket_balance_keys, h_leaf);
...
void multi_prefix_sort_1thread_noleaf(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"multigpu prefix sort 1thread noleaf"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
