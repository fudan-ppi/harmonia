...
using namespace AAA;
__global__ void range_prefix_sort_1thread_search_1_thread_scan_kernel(Inner_node *d_innode, BLeaf_node *d_bleafnode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys_begin, key_t *d_keys_end,int *d_pos, int tree_height, GPU_Result *d_gresult, int key_count, int bleaf_count){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            key_t *tmp_dkeys_begin = d_keys_begin[iii] + stride;
            key_t *tmp_dkeys_end = d_keys_end[iii] + stride;
...
#endif
            range_prefix_sort_1thread_search_1_thread_scan_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_bleafnode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, tmp_dkeys_begin_after, tmp_dkeys_end,tmp_dpos_after,  kernel_height , tmp_d_gresult, Para_Search_Bucket, bleaf_count);
...
void range_prefix_sort_1thread_search_and_1thread_scan(BPlusTree &tree,ifstream &search_file){
/*{{{*/
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
    ...
    for (int iii=0; iii<ngpus; iii++) {
        ...
        //gpu_malloc
        CUDA_ERROR_HANDLER(cudaMalloc(&(d_keys_begin[iii]), batch_d_key_size*2));
        CUDA_ERROR_HANDLER(cudaMalloc(&(d_keys_end[iii]), batch_d_key_size*2));
    ...
    cout<<"range: prefix-sort-1thread-search &  1thread-scan"<<endl;
