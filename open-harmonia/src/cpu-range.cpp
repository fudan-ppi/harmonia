...
void search_range_cpu_prefetch_sort_NTG(ifstream &file, BPlusTree &tree){
/*{{{*/
    ...
    vector<int> leafnode_id(keys_size);
    ...
    else cout<<"HB+ range [no gpu]: cpu prefetch sort avxNTG"<<endl;
    ...
    cout<<"Total time:      "<<tt+sort_time+traverse_time<<endl;
...
void traverse_PPItree(key_t *start, key_t *end, int *leafnode_id,int *relist_idx, result_t(* values_range)[8], BPlusTree &tree){
    ...
    for (int i = 0; i < SWP_P2; i++) {
        ...
        int bleaf_pos = leafnode_id[i];
        
        BLeaf_node *node = tree.getBLeafNodeByIdx(bleaf_pos); 
...
void search_cpu_prefetch_NTG_PPI_wo_leaf(key_t* keys, int* leafnode_id, int* relist_idx, BPlusTree &tree){
    ...
    for(int i=0;i<SWP_P;i++) {
        leafnode_id[i] = tree.getInnerNodeIdx(nodes[i])-innerSize_wo_last_inner_node;
    }
   /*}}}*/
