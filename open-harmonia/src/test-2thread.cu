...
__global__ void test_2thread_kernel(Inner_node *d_innode,  int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
    ...
    //-----------------------------------------------------------------------------------------
            ...
            if(search_idx == 0){
                if(i == tree_height-1){
                    ...
                    if(isBalance){
                        result.blfnode = (int)(BLeaf_node *)node->child[begin+8];
            ...
            if(selfFlag == 1 && flag[r][search_idx] == 0){
                if(i==tree_height-1){
                    if(isBalance){
                        result.blfnode = (int)(BLeaf_node *)node->child[idx + nexthalf[r]*2];
                        result.relist_idx = 111;
                   }else{ 
                    ...
                    start_idx[cur_r] = (int)node->child[idx + nexthalf[r]*2];
            }
...
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        test_2thread_kernel<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,  rootIdx, d_keys+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
void test_2thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"test-2thread-no-sort-noprefix]"<<total * Para_Search_Bucket<<endl;
