...
namespace{
    ...
    //prefix opt    
    __constant__ unsigned short dc_prefix_up[4161]; //1+64+64*64
    unsigned short *d_prefix_down;
    __constant__ unsigned short dc_base[1200];   //node_num / 1024; including last_inner_node which is  unnecessary
    int upNum;
/*}}}*/
}
...
__global__ void ppi_bpt_V14_prefix_opt(Inner_node *d_innode, unsigned short* d_prefix_down, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys, int upNum){
    /*{{{*/ 
...
*/
    ...
    //-----------------------------------------------------------------------------------------
            ...
            int tmp_prefix;
            if (up_prefix) 
                tmp_prefix = (int)dc_prefix_up[pos] + (int)dc_base[pos/BASE_GRP]*BASE_COEF;
            else if (i==tree_height-1) {}    //forget the balance
            else tmp_prefix = ((int)(dc_base[pos/BASE_GRP]) * BASE_COEF + (int)(__ldg(&d_prefix_down[pos-upNum]))) ; 
            //===== shit
            if(search_idx == 0){
                if(i == tree_height-1){
                    //result.blfnode = (BLeaf_node *)node->child[0];
                    if(isBalance){
                        result.blfnode = tmp_prefix+(begin+8) ;
                    ...
                    start_idx[cur_r] = tmp_prefix + (begin+8);
            }
            //=====
            ...
            if(selfFlag == 1 && flag[r][search_idx] == 0){
                if(i==tree_height-1){
                    if(isBalance){
                        result.blfnode = tmp_prefix+idx+nexthalf[r]*2 ;
                        result.relist_idx = 111;
                   }else{ 
                    ...
                    start_idx[cur_r] = tmp_prefix + (idx + nexthalf[r]*2);
            }
...
*
*   
*           PPI_BPT_Search_GPU_V14_3_prefix_opt
*           double buffer
*           using 2 thread
*           batch
...
static void prepareGPU_v3(BPlusTree tree){
    //transfer innode
    ...
    unsigned short *baseArray;

    tree.getSplitPrefixArray(prefixArray, baseArray);
 
    
    upNum = tree.getLevelIdx(3);   // root: level 0, 取level 3（第四层）的起始idx，即前三层的node个数
    int size = tree.getInnerSize();
    int downNum = size - upNum;
    
    d_prefix_down = (unsigned short *)transferArrayToGPU((int*)(prefixArray+upNum),downNum * sizeof(unsigned short));
    
    cudaMemcpyToSymbol(dc_prefix_up, prefixArray, upNum * sizeof(unsigned short));
    cudaMemcpyToSymbol(dc_base, baseArray, size/BASE_GRP * sizeof(unsigned short));
...
#endif
        ppi_bpt_V14_prefix_opt<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix_down, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys,upNum);
...
void PPI_BPT_Search_GPU_V14_3_prefix_opt(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
    ...
    cout<<"Para: "<<Para_Search_Bucket<<endl;
    //cout<<nums<<endl;
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V14_3_prefix_opt: split prefixArray"<<total * Para_Search_Bucket<<endl;
