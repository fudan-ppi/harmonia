...
using namespace AAA1;
__global__ void range_hb_kernel(Inner_node *d_innode, BLeaf_node *d_bleafnode, int root_idx,  key_t *d_keys_begin, key_t *d_keys_end, int tree_height, GPU_Result *d_gresult, int key_count){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            range_hb_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_bleafnode[iii], rootIdx, tmp_dkeys_begin, tmp_dkeys_end, kernel_height , tmp_d_gresult, Para_Search_Bucket);
...
void range_hb(BPlusTree &tree,ifstream &search_file){
/*{{{*/
