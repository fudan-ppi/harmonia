...
namespace{
    ...
    int Para_Search_Bucket = (Block_Per_Grid * Thread_Per_Block) / PPI_Thread_Per_Query * GPU_SWP_SIZE;//851968
...
__global__ void test_prefix_8thread_kernel(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
*
*   
*           TEST-PREFIX-ONLY-8THREAD-NO-SORT
*           no sort
*           double buffer
*           using 8 thread
*           batch
...
*           new tree 
*           CPU multi-thread
*           load-balance ;cpu down;
*           *thread_scheduling
*
*-----------------------------------------------------------------------------*/

static void* launch_kernel_thread_new_tree_no_sort(void *args){
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        test_prefix_8thread_kernel<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
void test_prefix_8thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
    ...
    pthread_t tid;
    if((pthread_create(&tid,NULL,launch_kernel_thread_new_tree_no_sort,NULL))!=0){
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"test-prefix-8thread no sort"<<total * Para_Search_Bucket<<endl;
