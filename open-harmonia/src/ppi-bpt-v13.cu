...
//for cache
#define PRE_PROCESS_THREAD_NUM 12
#define BIT_FLAG (0xfff)         //1111'b    PRE_PROCESS_THREAD_NUM个1组成BIT_FLAG
#define PROFILE_TIME 1
#define FORCE_PROFILE_PART (0.1)    //总数据的后(1-FORCE_PROFILE_PART)必须是经过PROFILE的
#define EMPTY_PROFILE_TIME 5000    //因为非堵塞的GET，对于前EMPTY_PROFILE_TIME的空数据不被算作profile time


#define thread_52
#ifdef thread_52
#define PROCESS_THREAD_NUM (52-PRE_PROCESS_THREAD_NUM)
#else 
#define PROCESS_THREAD_NUM (26-PRE_PROCESS_THREAD_NUM)
#endif
...
namespace{
    ...
    volatile int data_flag[PRE_PROCESS_THREAD_NUM];//data flag for pre process 
    volatile int available_bucket_number[PRE_PROCESS_THREAD_NUM];   //对于每一个预处理线程，记录new_host_keys分别有多少bucket
    volatile int profile_thread = 0;

    HashCache2 hashmap;

    int Thread_Per_Block = PPI_Block_Dim_2thread;
    int Block_Per_Grid = 13 * PPI_Block_Per_SM ;
    int Para_Search_Bucket = (Block_Per_Grid * Thread_Per_Block) / PPI_Thread_Per_Query_2thread * GPU_SWP_SIZE2;//851968
...
__global__ void ppi_bpt_V13_search_kernel_2threads_new_tree(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
*           *thread_scheduling
*           
*           cpu_cache_profile_version 
*           single thread
*
*-----------------------------------------------------------------------------*/

void* launch_kernel_thread_new_tree_measure_v13(void *args){
/*{{{*/
    ...
    /*
       done_flag_local用于判定预处理线程是否都处理完了全部数据
       done_flag_local 每一个bit代表一个线程，该bit为1,则该线程处理完毕
     */
    int done_flag_local = 0;
    int continue_flag = 0;
    ...
    while (1){
        ...
        //printf("launch:  id: %d, available_bucket_number:  %d\n", i,  available_bucket_number[i%PRE_PROCESS_THREAD_NUM]);
        //数据未处理好
        while (data_flag[i%PRE_PROCESS_THREAD_NUM]<i) {
            
            //已无新的数据会再进来
            if (available_bucket_number[i%PRE_PROCESS_THREAD_NUM] < i) {
                done_flag_local |= (1<<(i%PRE_PROCESS_THREAD_NUM)); //完成的线程对应的bit置1, 就算重复做也没有关系

                if (done_flag_local & (BIT_FLAG) == BIT_FLAG) continue_flag = 2;  //全完成
                else {
                    continue_flag = 1;
        ...
        if (continue_flag == 2) break;

        CUDA_ERROR_HANDLER(cudaMemcpyAsync(d_keys + stride, new_host_keys + i * Para_Search_Bucket, batch_d_key_size, cudaMemcpyHostToDevice,stream[idx]));
        ...
        ppi_bpt_V13_search_kernel_2threads_new_tree<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
static void * dispatcher(void *args){
 /*{{{*/
    ...
    while (1){
        ...
        //以下的一段判断是否做完的方法和在launch gpu时的类似
        while(stream_flag[idx] < i) {
            //printf("dispatcher: id: %d,  stream_flag: %d,  available_bucket_number:  %d\n", i, stream_flag[idx], available_bucket_number[i%PRE_PROCESS_THREAD_NUM]);
            if (available_bucket_number[i%PRE_PROCESS_THREAD_NUM] < i) {
                done_flag_local |= (1<<(i%PRE_PROCESS_THREAD_NUM)); //完成的线程对应的bit置1, 就算重复做也没有关系
     ...
     //-------------------------------------------------------------------------
        ...
        for(int j=0;j<Para_Search_Bucket;j+= PROCESS_CHRUNK_SIZE ){
            int idx= start+j;
            key_t key = new_host_keys[start + host_pos[idx]];
...
/*{{{*/
    ...
    vector<value_t> vals(PROCESS_CHRUNK_SIZE);

    int profile_time = -1;
    int block_time = 0;
    while(1){
        //printf("threadID: %d, profile_thread %d, profile_time %d\n",threadID, profile_thread,profile_time);
        if (threadID==profile_thread){
            
            profile_time++;
            if(profile_time==PROFILE_TIME) {
                __sync_add_and_fetch(&(profile_thread),1);
                //profile_thread++;
            }
            //cout<<"profile_thread"<<profile_thread<<endl; 
        }
        ...
        tuple<int,int,int> range = buffers[buff_idx].get_unblock(idx);
        ...
        if (start==-2) {
            //printf("start==-2 threadID: %d, profile_thread %d, profile_time %d\n",threadID, profile_thread,profile_time);
            if ((profile_thread==threadID)&&(block_time<EMPTY_PROFILE_TIME)) {
                block_time++;
                profile_time--;
            }
            continue;
        ...
        if(start == -1) {
            if (profile_thread<threadID) {
                printf("thread %d cannot stop now!\n", threadID);
                while (profile_thread<threadID);
                profile_thread++;
            }
            return NULL;
        }
        ...
        for(int k = start,j=0;k <=end;k++,j++){ 
            ...
            key_t key = new_host_keys[bucket_base + host_pos[k]];
            ...
            if(h_gresult[k].relist_idx == 111){ 
                ...
                if (threadID==profile_thread) hashmap.addCache(key, vals[j]);
            }else{
...
static void *data_preprocess(void *args) {
    int threadId = (long long)args;
    stick_this_thread_to_core2(threadId+2+2+PROCESS_THREAD_NUM);
    
    vector<value_t> vals(Para_Search_Bucket);


    int taskId = threadId;
    int newTaskId = threadId;
    int newStart = newTaskId * Para_Search_Bucket;
    int newI = 0;

    while (taskId < total) {
         
        //asm volatile("":::"memory");    //important!
        //强行等到PROFILE结束
        if ((profile_thread <PROCESS_THREAD_NUM)&&(taskId > (total * FORCE_PROFILE_PART)  ))
            while (profile_thread<PROCESS_THREAD_NUM);
                //cout<<"wait"<<endl ;
        
        asm volatile("":::"memory");    //important!

        int start = taskId * Para_Search_Bucket;
        for (int i=0;i<Para_Search_Bucket;i++) {
            
            key_t key = host_keys[start+i];
          
#ifdef PROFILE_ONLY
            
            if ((profile_thread< PROCESS_THREAD_NUM )||(hashmap.lookup(key, vals[i])==-1)||(true))
                
#else
            //profile时不会查询
            if ((profile_thread< PROCESS_THREAD_NUM )||(hashmap.lookup(key, vals[i])==-1))
#endif             
            {    
                new_host_keys[newStart + newI] =  key;   
                newI++;
                if (newI==Para_Search_Bucket) {
                    //inform
                    data_flag[threadId] = newTaskId;

                    
                    newTaskId += PRE_PROCESS_THREAD_NUM;
                    newStart = newTaskId * Para_Search_Bucket;
                    newI = 0;
                }
...
#ifdef ENABLE_TEST
                printf("%lld: %lld\n", key, vals[i]);
                //cout<<key<<":"<< vals[i]<<endl;
#endif
            }
        }
        taskId += PRE_PROCESS_THREAD_NUM;
    
    }
    if (newI!=0) {
        data_flag[threadId] = newTaskId;
    }
    available_bucket_number[threadId] = data_flag[threadId];
   
    printf("newTaskId: %d ", newTaskId);
    printf("hits: %d\n",hashmap.hits);
}



void PPI_BPT_Search_GPU_V13_thread_scheduling_cpu_cache_profile_version(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
    ...
    host_keys = (key_t*)malloc(sizeof(key_t)*Keys_Count);
    CUDA_ERROR_HANDLER(cudaMallocHost(&new_host_keys, sizeof(key_t)*Keys_Count));
    ...
    cout<<"Para_Search_Bucket: "<<Para_Search_Bucket<<endl;
    //cout<<nums<<endl;
    ...
    //data flag init 
    for (int i=0; i<PRE_PROCESS_THREAD_NUM;i++) {
        data_flag[i] = -1;
        available_bucket_number[i] = INT_MAX; 
    }
    ...
    pthread_t tid;
    if((pthread_create(&tid,NULL,launch_kernel_thread_new_tree_measure_v13,NULL))!=0){
    ...
    //cpu_cache data_preprocess
    for (int i=0; i<PRE_PROCESS_THREAD_NUM;i++) {
        if (pthread_create(&tid, NULL, data_preprocess, (void *)i)!=0) {
            cout<<"can't create thread\n"<<endl;
        }
    }
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V13 [sort key first,2threads,doublebuffer, whole, new_tree,load balance(), cpu_cache_profile_version,  single gpu]"<<total * Para_Search_Bucket<<endl;
    ...
    CUDA_ERROR_HANDLER(cudaFreeHost(new_host_keys));
