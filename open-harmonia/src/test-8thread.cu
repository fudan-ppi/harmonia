...
__global__ void test_8thread_kernel(Inner_node *d_innode, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/
...
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        test_8thread_kernel<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode, rootIdx, d_keys+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
void test_8thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"test-8thread-old_tree "<<total * Para_Search_Bucket<<endl;
