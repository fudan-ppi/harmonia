...
#define  GPU_SWP_SIZE8 (16*4) 


#define PPI_Block_Dim_8thread 128
//#define PPI_Block_Per_SM 64

#define PPI_Thread_Per_Query_8thread 8
#define  Sort_Per_8Thread (GPU_SWP_SIZE8 / PPI_Thread_Per_Query_8thread)
...
/*{{{*/
    ...
    int Thread_Per_Block = PPI_Block_Dim_8thread;
    int Block_Per_Grid = SM * PPI_Block_Per_SM ;
    int Para_Search_Bucket = (Block_Per_Grid * Thread_Per_Block) / PPI_Thread_Per_Query_8thread * GPU_SWP_SIZE8;//851968
...
*/
    ...
    cudaEvent_t **g_sort_start;
    cudaEvent_t **g_sort_stop;
    cudaEvent_t **g_search_start;
    cudaEvent_t **g_search_stop;
...
__global__ void bit_multi_prefix_sort_8thread_kernel(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
    int key_base = PPI_Block_Dim_8thread / PPI_Thread_Per_Query_8thread * GPU_SWP_SIZE8;
    int key_idx = key_base * blockIdx.x + threadIdx.x/ PPI_Thread_Per_Query_8thread;  

    if(key_idx>=key_count) return;

    int search_idx = threadIdx.x % PPI_Thread_Per_Query_8thread;
    int query_idx_in_local_block = threadIdx.x / PPI_Thread_Per_Query_8thread;
    const int r = query_idx_in_local_block;//just for simple

    const int row = PPI_Block_Dim_8thread/ PPI_Thread_Per_Query_8thread;//blockDim.x/ PPI_Thread_Per_Query;
    const int row_swp = row * GPU_SWP_SIZE8;
    __shared__ char flag[row][9];
    __shared__ int inner_index_result[row];
    __shared__ int start_idx[row_swp];


    int stride = PPI_Block_Dim_8thread / PPI_Thread_Per_Query_8thread;
...
*/
    ...
    for (int k = 0; k<Sort_Per_8Thread; k++){
...
/*{{{*/
    ...
    g_sort_start = (cudaEvent_t **)malloc(ngpus * sizeof(cudaEvent_t*));
    g_sort_stop = (cudaEvent_t **)malloc(ngpus * sizeof(cudaEvent_t*));
    g_search_start = (cudaEvent_t **)malloc(ngpus * sizeof(cudaEvent_t*));
    g_search_stop = (cudaEvent_t **)malloc(ngpus * sizeof(cudaEvent_t*));
    ...
    for (int iii=0;iii<ngpus;iii++) {
        ...
        g_sort_start[iii] = (cudaEvent_t *)malloc(total_per_gpu * sizeof(cudaEvent_t));
        g_sort_stop[iii] = (cudaEvent_t *)malloc(total_per_gpu * sizeof(cudaEvent_t));
        g_search_start[iii] = (cudaEvent_t *)malloc(total_per_gpu * sizeof(cudaEvent_t));
        g_search_stop[iii] = (cudaEvent_t *)malloc(total_per_gpu * sizeof(cudaEvent_t));
        for (int i=0;i<total_per_gpu;i++) {
            cudaEventCreate(&(g_sort_start[iii][i]));
            cudaEventCreate(&(g_sort_stop[iii][i]));
            cudaEventCreate(&(g_search_start[iii][i]));
            cudaEventCreate(&(g_search_stop[iii][i]));
        }
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            cudaEventRecord(g_sort_start[iii][i], stream[iii][idx]); 
            cub::DeviceRadixSort::SortPairs(d_temp_storage[iii] + idx*temp_storage_bytes, temp_storage_bytes, tmp_dkeys, d_keys_after[iii]+stride, tmp_dpos, d_pos_after[iii]+stride, Para_Search_Bucket,start_bit, end_bit,stream[iii][idx]);
            cudaEventRecord(g_sort_stop[iii][i], stream[iii][idx]); 

            cudaEventRecord(g_search_start[iii][i], stream[iii][idx]); 
            bit_multi_prefix_sort_8thread_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys_after[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket, bucket_balance_keys);
            cudaEventRecord(g_search_stop[iii][i], stream[iii][idx]); 
...
void bit_multi_prefix_sort_8thread(BPlusTree &tree,ifstream &search_file, int start_bit1, int end_bit1){
/*{{{*/
...
// 
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
    ...
    cub::DeviceRadixSort::SortPairs(d_temp_storage[0], temp_storage_bytes, d_keys[0], d_keys_after[0], d_pos[0],d_pos_after[0],Para_Search_Bucket,start_bit, end_bit);
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"bit multigpu prefix sort 8thread"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
    ...
    for (int i=0;i<ngpus;i++) {
        ...
        float sort_time = 0;
        float search_time = 0;
        for (int j=0;j<total_per_gpu;j++) {
            cudaEventElapsedTime(&time_gpu,g_sort_start[i][j],g_sort_stop[i][j]);
            sort_time += time_gpu;
            cudaEventElapsedTime(&time_gpu,g_search_start[i][j],g_search_stop[i][j]);
            search_time += time_gpu;
        }
        cout<<"GPU sort time:   "<<sort_time/1000<<endl;
        cout<<"GPU search time:   "<<search_time/1000<<endl;
