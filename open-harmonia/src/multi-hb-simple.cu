      ...
      /*{{{*/
        ...
        for(int i=0;i<total_per_gpu;i++){
            ...
            for (int iii=0;iii<ngpus;iii++) {
                ...
                bpt_search_kernel_balance<<<grid_dim,block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_cur_inner_node[iii] + idx*Para_Search_Bucket,inner_pool_start,d_keys[iii]+idx*Para_Search_Bucket,tree_height,d_gresult[iii]+idx*Para_Search_Bucket,Para_Search_Bucket);
...
void HB_simple(BPlusTree &tree,ifstream &search_file){
/*{{{*/
    //cudaProfilerStart();
    ...
    //HB paper中是做了BALANCE的，所以前几层是在CPU上做的，现在由于完全不要CPU了，所以理论上不用cur_inner_node会更好一些，只使用root就好了，可以省去在GPU上的访存。。 
    //init cur_inner_node
    for (int iii=0;iii<ngpus;iii++) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    cout<<"HB+ tree no cpu simple version"<<endl;
