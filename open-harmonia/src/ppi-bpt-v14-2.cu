...
namespace{
    ...
    __constant__ int dc_prefix_up[16384];   //64KB int
    int *d_prefix_down;
...
__global__ void ppi_bpt_V14_prefix_up_full_constant_down_readonly(Inner_node *d_innode, int* d_prefix_down, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys, int upNum){
    /*{{{*/ 
...
*/
    ...
    //-----------------------------------------------------------------------------------------
            ...
            int tmp_prefix;
            if (pos<upNum) tmp_prefix = dc_prefix_up[pos];
...
*
*   
*           PPI_BPT_Search_GPU_V14_2_prefix_up_full_constant_down_readonly
*           double buffer
*           using 2 thread
*           batch
...
static void prepareGPU_v3(BPlusTree tree){
    ...
    if (downNum > 0){
        ...
        cudaMemcpyToSymbol(dc_prefix_up, prefixArray, tree.getInnerSize() * sizeof(int));
    }
...
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        ppi_bpt_V14_prefix_up_full_constant_down_readonly<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix_down, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys, upNum);
...
void PPI_BPT_Search_GPU_V14_2_prefix_up_full_constant_down_readonly(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V14_2 [based v9 prefix up full constant down readonly]"<<total * Para_Search_Bucket<<endl;
