...
#define LOAD_BALANCE_PECENTAGE 0.5//unit %; Range 0~1 ,0 means CPU compute 0% inner_node,100% means CPU compute compute all inner_node
...
namespace{
    ...
    //thread scheduling
    key_t *range;
    vector<Buffer> buffers(SPLIT_BUFFER_NUM);
    vector<ThreadToBuffer> thread_buffer_mapping(PROCESS_THREAD_NUM);
/*}}}*/
}
...
__global__ void ppi_bpt_V8_search_kernel_2threads_new_tree(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
void* launch_kernel_thread_new_tree_measure_v8(void *args){
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        ppi_bpt_V8_search_kernel_2threads_new_tree<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
static void * dispatcher(void *args){
 /*{{{*/
    ...
    int maxxx=-1;
    int minnn=-1;
    for (int i=0;i<SPLIT_BUFFER_NUM-1;i++) {
        cout<<range[i]<<endl;
    }
    for(int i=0;i<total;i++){
        int idx = i%2;
        ...
        for(int j=0;j<Para_Search_Bucket;j+= PROCESS_CHRUNK_SIZE ){
            ...
            buffers[cur_idx].push(idx,idx + PROCESS_CHRUNK_SIZE - 1,i);
        }
        //thread_adjustment
...
*/
            ...
            if ((front.workload * 1.5 < back.workload)&&(buffers[front.bufferIdx].getThreadNum()>=4)) {
                for (int jj=0;jj<2;jj++) {
                    int thread_sche = buffers[front.bufferIdx].popThread();
                    buffers[back.bufferIdx].pushThread(thread_sche);
                    thread_buffer_mapping[thread_sche].setMapping(back.bufferIdx);
                    //cout<<"sche: thread: "<<thread_sche<<" from "<<front.bufferIdx<<" to "<<back.bufferIdx<<" at "<<i<<endl;
                }
            }
        }
...
/*{{{*/
    ...
    while(1){
        int buff_idx = thread_buffer_mapping[threadID].getMapping();
        tuple<int,int,int> range = buffers[buff_idx].pop();
...
void PPI_BPT_Search_GPU_V8_thread_scheduling(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
#ifdef thread_52
        thread_buffer_mapping[i].setMapping(i/2%SPLIT_BUFFER_NUM);
        buffers[i/2%SPLIT_BUFFER_NUM].pushThread(i);
#else
        thread_buffer_mapping[i].setMapping(i%SPLIT_BUFFER_NUM);
        buffers[i%SPLIT_BUFFER_NUM].pushThread(i);
#endif
    }
// 
    ...
    pthread_t tid;
    if((pthread_create(&tid,NULL,launch_kernel_thread_new_tree_measure_v8,NULL))!=0){
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V8 [sort key first,2threads,doublebuffer, whole, new_tree,load balance()]"<<total * Para_Search_Bucket<<endl;
