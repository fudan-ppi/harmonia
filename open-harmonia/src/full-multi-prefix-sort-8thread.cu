...
__global__ void full_multi_prefix_sort_8thread_kernel(Inner_node *d_innode, BLeaf_node *d_bleafnode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            full_multi_prefix_sort_8thread_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_bleafnode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys_after[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket);
...
void full_multi_prefix_sort_8thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
