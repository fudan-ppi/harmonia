...
#define CACHE_LENGTH (13 * PPI_Block_Per_SM * PPI_Block_Dim_2thread)
...
__global__ void ppi_bpt_V11_search_kernel_2threads_new_tree_cache(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count, cache_t *d_cache)
#else
__global__ void ppi_bpt_V11_search_kernel_2threads_new_tree_cache(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count)
#endif
{
    /*{{{*/ 
    ...
    __shared__ int start_idx[row];
    volatile __shared__ char nexthalf[row];

#ifdef ENABLE_CACHE
    volatile __shared__ char cache_hit_flag[row];
#endif
...
*/
    ...
    //先算一组Key的一层，再算下一层, 这样对于一组Key每一个Key都要有start_idx，且不能共用
    /*
    ...
    */
    ...
    //先算完一个Key，再算下一个Key，这样当进入的start_idx都是root_idx（即相同）时，可以一组Key共用一个start_idx
    //start_idx[threadIdx.x / PPI_Thread_Per_Query_2thread] = root_idx;
    ...
    for(int j=0;j<GPU_SWP_SIZE2;j++){
        ...
        //__syncthreads();
        //cache lookup
        cache_t &cache = d_cache[blockDim.x * blockIdx.x + threadIdx.x];
        cache.exist_time++;
        if ((target!=0)&&(cache.key == target)) {
            result.blfnode = cache.blfnode;
            result.relist_idx = cache.relist_idx;
            cache.exist_time = 0;
            cache_hit_flag[r] = 1;
        }
        //__syncthreads();
   
        if (cache_hit_flag[r] == 1) {
    //        printf("%lld, hit! %d, %d\n", target,blockIdx.x, threadIdx.x);
            continue;
        }
#endif
    ...
    //-----------------------------------------------------------------------------------------
            ...
            if(search_idx == 0){
                if(i == tree_height-1){
                    ...
                    start_idx[r] = __ldg(&d_prefix[pos]) + (begin+8);
            }
            //=====
...
#ifdef ENABLE_CACHE
                    if (cache.exist_time>CACHE_THRESHOLD) {
                        cache.key = target;
                        cache.blfnode = result.blfnode;
                        cache.relist_idx = idx + nexthalf[r]*2;
                        cache.exist_time=0;
                    }
#endif
                }else 
                    start_idx[r] = __ldg(&d_prefix[pos]) + (idx + nexthalf[r]*2);
            }
...
#ifdef ENABLE_CACHE
    cache_t **d_cache;
    int d_cache_size = sizeof(cache_t) * CACHE_LENGTH; 
...
#ifdef ENABLE_CACHE
            cache_t *d_cache_ptr = d_cache[iii]+idx*CACHE_LENGTH;
#endif
...
#ifdef ENABLE_CACHE
            ppi_bpt_V11_search_kernel_2threads_new_tree_cache<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys_after[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket, d_cache_ptr);
#else 
            ppi_bpt_V11_search_kernel_2threads_new_tree_cache<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys_after[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket);
#endif
...
void PPI_BPT_Search_GPU_V11_multigpu_basedv5_cache(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
#ifdef ENABLE_CACHE
    d_cache = (cache_t **)malloc(ngpus * sizeof(cache_t*));

    for (int iii=0;iii<ngpus;iii++) {
        cudaSetDevice(iii);
        CUDA_ERROR_HANDLER(cudaMalloc(&(d_cache[iii]), d_cache_size * 2 ));
    }
#endif
    ...
    cout<<"GPU PPI V11 [sort key first,2threads,doublebuffer, whole, new_tree, multi-gpu, gpu_cache]"<<total * Para_Search_Bucket<<endl;
...
#ifdef ENABLE_CACHE
        CUDA_ERROR_HANDLER(cudaFree(d_cache[i]));
#endif
    }
