...
#define PRE_PROCESS_THREAD_NUM 16
#define BIT_FLAG (0xffff)          //1111'b
...
__global__ void ppi_bpt_V12_search_kernel_2threads_new_tree(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
void* launch_kernel_thread_new_tree_measure_v12(void *args){
/*{{{*/
    ...
    while (1){
        ...
        ppi_bpt_V12_search_kernel_2threads_new_tree<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
static void *data_preprocess(void *args) {
    ...
    while (taskId < total) {
        ...
        for (int i=0;i<Para_Search_Bucket;i++) {
            ...
            if (hashmap.lookup(key, vals[i])==-1){
...
void PPI_BPT_Search_GPU_V12_thread_scheduling_cpu_cache(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
    ...
    pthread_t tid;
    if((pthread_create(&tid,NULL,launch_kernel_thread_new_tree_measure_v12,NULL))!=0){
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V12 [sort key first,2threads,doublebuffer, whole, new_tree,load balance(), cpu_cache,  single gpu]"<<total * Para_Search_Bucket<<endl;
