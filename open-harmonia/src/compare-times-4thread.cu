...
__global__ void compare_times_4thread_kernel(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys, unsigned long long* compare_times){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        compare_times_4thread_kernel<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys, d_compare_times);
...
void compare_times_4thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"4thread idx compare times        "<<(unsigned long long)compare_times[0]<<endl;
    cout<<"4thread idx&key compare times    "<<(unsigned long long)compare_times[0]+compare_times[1]<<endl;
