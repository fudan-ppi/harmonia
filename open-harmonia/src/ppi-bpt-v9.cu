...
__global__ void ppi_bpt_V9_search_kernel_2threads_new_tree(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
void* launch_kernel_thread_new_tree_measure_v9(void *args){
/*{{{*/
    ...
    for(int i=0;i<total;i++){
        ...
        ppi_bpt_V9_search_kernel_2threads_new_tree<<<grid_dim, block_dim,0,stream[idx]>>>(d_innode,d_prefix, inner_node_size_wo_last_inner_node, rootIdx, d_keys_after+stride, kernel_height , d_gresult+stride, Para_Search_Bucket, bucket_balance_keys);
...
void PPI_BPT_Search_GPU_V9_thread_scheduling(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
    ...
    pthread_t tid;
    if((pthread_create(&tid,NULL,launch_kernel_thread_new_tree_measure_v9,NULL))!=0){
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"GPU PPI V9 [sort key first,2threads,doublebuffer, whole, new_tree,load balance()]"<<total * Para_Search_Bucket<<endl;
