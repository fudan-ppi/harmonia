...
__global__ void multi_prefix_nosort_1thread_kernel(Inner_node *d_innode, int* d_prefix, int inner_node_size_wo_last_inner_node, int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            multi_prefix_nosort_1thread_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii],d_prefix[iii], inner_node_size_wo_last_inner_node, rootIdx, d_keys[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket, bucket_balance_keys);
...
void multi_prefix_nosort_1thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"multigpu prefix nosort 1thread"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
