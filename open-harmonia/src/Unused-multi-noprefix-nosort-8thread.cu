...
__global__ void multi_noprefix_nosort_8thread_kernel(Inner_node *d_innode,  int root_idx,  key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count,int d_bucket_balance_keys){
    /*{{{*/ 
...
*/
    ...
    //-----------------------------------------------------------------------------------------
            ...
            if(selfFlag == 1 && flag[r][search_idx] == 0){
                if(i==tree_height-1){
                    if(isBalance){
                        result.blfnode = (int)(BLeaf_node *)node->child[idx];
                        result.relist_idx = 111;
                   }else{ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            multi_noprefix_nosort_8thread_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_innode[iii], rootIdx, d_keys[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket, bucket_balance_keys);
...
void multi_noprefix_nosort_8thread(BPlusTree &tree,ifstream &search_file){
/*{{{*/
...
// 
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
   ...
   total_time += (end.tv_sec - start.tv_sec) + (end.tv_usec-start.tv_usec) / 1000000.0;  
    ...
    cout<<"multigpu noprefix nosort 8thread"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
