...
using namespace noprefix_nosort_RB;
//---------------------------------------------------------------
//  leaf: height=0

__global__ void multi_noprefix_nosort_RB_kernel(kArr_t *d_node, cArr_t *d_child, int root_idx, key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            multi_noprefix_nosort_RB_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_node[iii],d_child[iii], rootIdx, d_keys[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket);
...
void multi_noprefix_nosort_RB(RB &tree, ifstream &search_file){
/*{{{*/
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
    ...
    //----------------------------------------------- 
    cout<<"multigpu noprefix nosort_RB 1/"<<Narrow_Rate<<" thread"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
