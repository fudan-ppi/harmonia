...
using namespace half_prefix_nosort;
//---------------------------------------------------------------
//  leaf: height=0
//  *d_leaf_record only contains leaf's child record, so it starts from internal_node_num of pointer section
__global__ void half_multi_prefix_nosort_kernel(kArr_t *d_node, int* d_prefix, cArr_t *d_leaf_record, int root_idx, int internal_node_num, key_t *d_keys, int tree_height, GPU_Result *d_gresult, int key_count){
    /*{{{*/ 
...
/*{{{*/
    ...
    for(int i=0;i<total_per_gpu;i++){
        for (int iii=0;iii<ngpus;iii++) {
            ...
            half_multi_prefix_nosort_kernel<<<grid_dim, block_dim,0,stream[iii][idx]>>>(d_node[iii],d_prefix[iii],d_leaf_record[iii], rootIdx, internal_node_num, d_keys[iii]+stride, kernel_height , d_gresult[iii]+stride, Para_Search_Bucket);
...
void half_multi_prefix_nosort(CFB &tree, ifstream &search_file){
/*{{{*/
  ...
  // 这里使用nKeys_Count(而不是nums/ngpus)是为了使得在nums不是Keys_Count大小时依然在结果上正确。因为任务不是按照保证每个GPU做同量分配的，而是保证前面的gpu可以占满
    ...
    //----------------------------------------------- 
    cout<<"half multigpu prefix nosort 1/"<<Narrow_Rate<<" thread"<<ngpus * total_per_gpu * Para_Search_Bucket<<endl;
