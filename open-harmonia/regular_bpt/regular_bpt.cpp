...
void find_and_print(node * root, key_t key, bool verbose) {
	...
	if (r == NULL)
		...
		printf("Record at %lx -- key %lld, value %lld.\n",
 ...
 */
void find_and_print_range( node * root, key_t key_start, key_t key_end,
		bool verbose ) {
	int i;
	key_t array_size = key_end - key_start + 1;
	key_t returned_keys[array_size];
	...
	if (!num_found)
		...
		for (i = 0; i < num_found; i++)
			printf("Key: %lld   Location: %lx  Value: %lld\n",
					returned_keys[i],
...
node * make_node( void ) {
    ...
    new_node->generate_time = generate_time;
    generate_time++;
 ...
 */
node * coalesce_nodes(node * root, node * n, node * neighbor, int neighbor_index, key_t k_prime) {
